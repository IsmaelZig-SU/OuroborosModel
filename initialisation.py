...
class Initialisation : 
	...
	def __init__(self, model, X_true, nobs, Re, ens) :
		...
		Parameters : 
			...
			Re : Parameter Reynolds number
			ens : Ensemble cardinality

		Returns : 

			H (np.ndarray) : Downsampling matrix (sensor locations) [nobs, dim]
			Y_obs (np.ndarray) : High fidelity observations (X_true@H.T) [T, nobs]
			Psi_f (np.ndarray): initial forecast ensemble [ens, T, D]
			X_f (np.ndarray): Initial forecast mean [T,D]
	...
	def Psi_f(self) : 
		...
		initial_condition = torch.tensor(self.X_true[0:1,:], dtype = torch.float32).to(self.device)
